...
object LogProcessingApp5 {
  ...
  def main(args: Array[String]) {
    ...
    println("Launching LogProcessingApp exercise 5...")
    ...
    val requestsByHost = webEvents.map(x => (x.sourceHost, 1)) // Convert to tuple (SourceHost, 1)
    ...
    requestsByHost.foreachRDD(rdd => {
      println()
      println(dateFormat.format(Calendar.getInstance().getTime()) + "-------------------------------------------------")
      rdd.foreach(x => println("Host:" + x._1 + ", requests= " + x._2))
    })

    requestsByHost.foreachRDD(rdd => {
      println()
      println("Top 5 hosts by requests:")
      rdd.sortBy(x => x._2, false).take(5).foreach(x => println(x))
    })

    authEvents.foreachRDD(rdd => print()) //To avoid errors
