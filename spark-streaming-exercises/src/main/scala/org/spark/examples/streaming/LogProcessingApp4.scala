...
object LogProcessingApp4 {
  ...
  def main(args: Array[String]) {
    ...
    println("Launching LogProcessingApp exercise 4...")
    ...
    authEvents.filter(x => x.message.equals(AUTH_FAIL)) // Keep only the auth fails
      ...
      .transform(rdd => rdd.join(knownHosts).filter(x => x._2._2 == 0).map(x => (x._1,x._2._1))) //Filter known hosts and keep format
      .foreachRDD(rdd => {
